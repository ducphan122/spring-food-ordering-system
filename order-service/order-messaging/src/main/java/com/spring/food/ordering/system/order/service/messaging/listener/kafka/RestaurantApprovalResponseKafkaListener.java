...
import com.spring.food.ordering.system.order.service.domain.ports.input.message.listener.restaurantapproval.RestaurantApprovalResponseMessageListener;
    ...
    public void receive(
        ...
        messages.forEach(avroModel -> {
            if (avroModel.getBefore() == null && DebeziumOp.CREATE.getValue().equals(avroModel.getOp())) {
                ...
                RestaurantOrderEventPayload restaurantOrderEventPayload = kafkaMessageHelper.getOrderEventPayload(
                ...
                try {
                    if (OrderApprovalStatus.APPROVED
                        ...
                        log.info(
                                "Processing approved order for order id: {}", restaurantOrderEventPayload.getOrderId());
                        ...
                        log.info(
                                ...
                                String.join(
                                        FAILURE_MESSAGE_DELIMITER, restaurantOrderEventPayload.getFailureMessages()));
